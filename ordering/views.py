...
class LinePayHandler:
    ...
    def request_payment(self, order, confirm_url):
        ...
        payload = {
            ...
            "packages": [
                {"id": f"PKG_{order.id}", "amount": order.total, "products": products}
            ],
            "redirectUrls": {"confirmUrl": confirm_url, "cancelUrl": confirm_url},
        }
        ...
        try:
            with transaction.atomic():
                ...
                if payment_method == "linepay":
                    ...
                    confirm_url = (
                        f"{protocol}://{host}/api/orders/line_callback/?oid={order.id}"
                    )
